#!/usr/bin/env python3
"""
Standalone MCP server for Build Hub.

Claude CLI spawns this as a subprocess (stdio transport).
It calls Build Hub's REST API at BUILDHUB_URL to answer queries about
structured project data. Also proxies knowledge context from Recall.

Usage:
    BUILDHUB_URL=http://127.0.0.1:8000 PROJECT_ID=abc python buildhub_mcp.py
"""
...
BUILDHUB_URL = os.environ.get("BUILDHUB_URL", "http://127.0.0.1:8000")
RECALL_URL = os.environ.get("RECALL_URL", "http://192.168.50.19:8200")
PROJECT_ID = os.environ.get("PROJECT_ID", "")

mcp = FastMCP("Build Hub")
...
async def get_project_summary(project_id: str = "") -> str:
    ...
    resp = await client.get(f"{BUILDHUB_URL}/api/projects/{pid}")
...
async def get_canvas_components(
    ...
    resp = await client.get(
        f"{BUILDHUB_URL}/api/projects/{pid}/canvases/{canvas_id}"
    )
    if resp.status_code != 200:
        return json.dumps({"error": f"HTTP {resp.status_code}", "detail": resp.text})
    data = resp.json()
...
async def list_canvases(project_id: str = "") -> str:
    ...
    if not pid:
        return json.dumps({"error": "No project_id provided"})
    client = await _get_client()
    resp = await client.get(f"{BUILDHUB_URL}/api/projects/{pid}/canvases")
    if resp.status_code != 200:
        return json.dumps({"error": f"HTTP {resp.status_code}", "detail": resp.text})
    return json.dumps(resp.json())
...
async def get_ideas(
    ...
    resp = await client.get(
        f"{BUILDHUB_URL}/api/projects/{pid}/ideas", params=params
    )
    if resp.status_code != 200:
        return json.dumps({"error": f"HTTP {resp.status_code}", "detail": resp.text})
    return json.dumps(resp.json())
...
async def search_ideas(project_id: str = "", query: str = "") -> str:
    ...
    resp = await client.post(
        f"{BUILDHUB_URL}/api/projects/{pid}/ideas/search",
        json={"query": query},
    )
    if resp.status_code != 200:
        return json.dumps({"error": f"HTTP {resp.status_code}", "detail": resp.text})
...
async def get_scaffold_job(
    ...
    client = await _get_client()
    resp = await client.get(
        f"{BUILDHUB_URL}/api/projects/{pid}/scaffold/jobs/{job_id}"
    )
    if resp.status_code != 200:
        return json.dumps({"error": f"HTTP {resp.status_code}", "detail": resp.text})
    return json.dumps(resp.json())
...
async def get_deploy_config(project_id: str = "") -> str:
    ...
    client = await _get_client()
    resp = await client.get(
        f"{BUILDHUB_URL}/api/projects/{pid}/deploy/environments"
    )
    if resp.status_code != 200:
        return json.dumps({"error": f"HTTP {resp.status_code}", "detail": resp.text})
    return json.dumps(resp.json())
...
async def get_knowledge_context(
    ...
    client = await _get_client()
    domain = f"build-hub:{project_slug}" if project_slug else None
